...
local function freezePlayer(id, freeze, isVisible)
    local player = id
    ...
    if not freeze then
        if not IsEntityVisible(ped) and isVisible then
        ...
        if IsEntityVisible(ped) and isVisible then
...
function SM:spawnPlayer(spawnIdx, cb)
    if spawnLock then
        return
    end
    ...
    CreateThread(function()
        ...
        if (not spawn) or (spawn.fadeOut) then
            DoScreenFadeOut(500)
        ...
        freezePlayer(PlayerId(), true, spawn.isVisible)
        ...
        RemoveAllPedWeapons(ped) -- TODO: give back from sql
        ...
        if IsScreenFadedOut() and (spawn.fadeIn) then
            DoScreenFadeIn(500)
        ...
        freezePlayer(PlayerId(), false, spawn.isVisible)
...
CreateThread(function()
    while true do
        ...
        if playerPed and playerPed ~= -1 then
            ...
            if autoSpawnEnabled or respawnForced then
                if NetworkIsPlayerActive(PlayerId()) then
                    if (diedAt and (math.abs(GetTimeDifference(GetGameTimer(), diedAt)) > 1000)) or respawnForced then
