...
Oxmysql is an alternative to the unmaintained oxmysql/ghmattimysql resources, utilising [node-mysql2](https://github.com/sidorares/node-mysql2) rather than [mysqljs](https://github.com/mysqljs/mysql).

As of v1.9.0 the preferred method of utilising oxmysql is via lib/MySQL, which can be loaded by adding `@oxmysql/lib/MySQL.lua` to your resource manifests. This resource should be 100% backwards compatible with oxmysql functionality on top of providing newer export wrappers and functionality.
...
- Support oxmysql syntax while providing newer (more accurate) names
...
```js
// JS
exports.gamemode.query('SELECT * from users WHERE identifier = ?', [identifier], (result) => {
  // callback response
})(async () => {
  const result = await exports.gamemode.query_async('SELECT * from users WHERE identifier = ?', [identifier]);
  // await a promise to resolve
})();
exports.gamemode.query_async('SELECT * from users WHERE identifier = ?', [identifier]).then((result) => {
